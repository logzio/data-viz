...
func TestMiddleWareSecurityHeaders(t *testing.T) {
	...
	Convey("Given the grafana middleware", t, func() {
		...
		middlewareScenario(t, "middleware should add correct Strict-Transport-Security header", func(sc *scenarioContext) {
			...
			sc.fakeReq("GET", "/api/").exec()
			So(sc.resp.Header().Get("Strict-Transport-Security"), ShouldEqual, "max-age=64000; includeSubDomains; preload") // LOGZ.IO GRAFANA CHANGE :: DEV-20823 Change string order
		})
	})
}
...
func TestMiddlewareContext(t *testing.T) {
	...
	Convey("Given the grafana middleware", t, func() {
		...
		middlewareScenario(t, "middleware should add X-Frame-Options header with deny for request when not allowing embedding", func(sc *scenarioContext) {
			sc.fakeReq("GET", "/api/search").exec()
			So(sc.resp.Header().Get("X-Frame-Options"), ShouldEqual, "DENY") // LOGZ.IO GRAFANA CHANGE :: DEV-20823 Change deny to capital
		})
...
func middlewareScenario(t *testing.T, desc string, fn scenarioFunc) {
	Convey(desc, func() {
		...
		sc.m.Use(AddSeceureResponseHeaders()) // LOGZ.IO GRAFANA CHANGE :: DEV-20823 Add new security headers
